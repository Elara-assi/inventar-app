...
export type BgaDictationFields = {
  object_type?: string;
...
/** Mappt generische Diktat-Slots auf das BGA-Erfassungsformular. */
export function toBgaFields(parsed: DictationResult): BgaDictationFields {
  const out: BgaDictationFields = {};
  if (parsed.object_type) out.object_type = parsed.object_type;
  const specification = [parsed.brand, parsed.model].filter(Boolean).join(" ");
  if (specification) out.specification = specification;
  if (parsed.serial_number) out.serial_number = parsed.serial_number;
  if (parsed.manufacturing_year) out.construction_year = String(parsed.manufacturing_year);
  if (parsed.condition) out.condition = parsed.condition;
  if (parsed.note) out.remark = parsed.note;
  return out;
}
